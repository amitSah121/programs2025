...
function keyReleased(e) {
  ...
  keyp = keyp.filter(
    (p) => p.toUpperCase() != current_key_released.toUpperCase()
  );
...
const set_key = function (key_code, key_) {
  ...
  if (
    ...
    (key_code >= 97 && key_code <= 124)
  ) {
    return key_; //.toUpperCase();
  } else if (
    (key_.charCodeAt(0) >= 32 && key_.charCodeAt(0) <= 64) ||
    (key_.charCodeAt(0) >= 91 && key_.charCodeAt(0) <= 96) ||
    (key_.charCodeAt(0) >= 125 && key_.charCodeAt(0) <= 128)
  ) {
    return key_;
  }
};
...
const key_combination = function (lst) {
  ...
  if (keyp.length >= lst.length) {
    for (let i = lst.length - 1; i >= 0; i--) {
      if (
        keyp.filter((p) => p.toUpperCase() == lst[i].toUpperCase()).length <= 0
      )
        return false;
    }
  } else return;
...
const key_combination_strict = function (lst) {
  ...
  if (keyp.length >= lst.length) {
    for (let i = lst.length - 1; i >= 0; i--) {
      if (
        keyp[keyp.length - i - 1].toUpperCase() !=
        lst[lst.length - i - 1].toUpperCase()
      )
        return false;
    }
  } else return;
...
let mouse_dx = 0,
  mouse_dy = 0;
let mouseKey = "",
  totalTrackPadFinger = 0;
let screenX = -1,
  screenY = -1,
  clientX,
  clientY;
let mouse_button_types = ["LEFT", "MIDDLE", "RIGHT"];
let mouse_pressed_events = [],
  mouse_released_events = [],
  mouse_moved_events = [],
  mouse_clicked_events = [],
  mouse_dragges_events = [],
  mouse_double_clicked_events = [];
...
let is_touch_started = false,
  is_touch_ended = false;
let touch_ended_count = 0;
let touch_started_events = [],
  touch_ended_events = [],
  touch_moved_events = [];
